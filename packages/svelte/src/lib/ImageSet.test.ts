...
describe('render', () => {
  test('with props', () => {
    ...
    assert.ok(
      container.querySelector('picture[class*="web-image-gen-picture"]')
    )
    ...
    Object.keys(set.formats).forEach((format) => {
      ...
      if (format === 'jpg') {
        tag = 'img'
        assert.ok(
          container.querySelector(`${tag}[class*="web-image-gen-img"]`)
        )
        ...
        assert.ok(
          container.querySelector(`${tag}[srcset="${set.placeholder}"]`)
        )
        assert.notOk(container.querySelector(`${tag}[width]`))
        assert.notOk(container.querySelector(`${tag}[height]`))
      } else {
        tag = 'source'
  ...
  test('with props: width', () => {
    const { container } = render(ImageSet, {
      set,
      width: 250,
    })

    assert.ok(container.querySelector(`img[width="250"]`))
    assert.notOk(container.querySelector(`img[height]`))
  })

  test('with props: height', () => {
    const { container } = render(ImageSet, {
      set,
      height: 250,
    })

    assert.notOk(container.querySelector(`img[width]`))
    assert.ok(container.querySelector(`img[height="250"]`))
  })
})
